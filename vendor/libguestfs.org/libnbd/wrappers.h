...
/* Older libnbd headers do not declare the 64-bit extent callback types,
 * but the generated Go bindings still mention them even when the matching
 * entry points are compiled out below. Provide placeholder declarations so
 * cgo can parse the bindings; the wrapper functions already return ENOTSUP
 * when these APIs are unavailable.
 */
