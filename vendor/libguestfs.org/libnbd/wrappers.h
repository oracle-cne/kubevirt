...
/*
 * Older libnbd headers do not define the extent64 callback types. The
 * generated wrappers below already guard the corresponding functions with
 * LIBNBD_HAVE_NBD_* checks and return a missing-function error when the
 * library lacks support, but the declarations still need these types to
 * compile.
 */
...
typedef struct {
  int (*callback) (void *user_data, const char *metacontext, uint64_t offset,
...
extern int chunk_callback (long *callbackid, void *subbuf, size_t count, uint64_t offset, unsigned int status, int *error);
...
extern int completion_callback (long *callbackid, int *error);


int _nbd_completion_callback_wrapper (void *user_data, int *error);
void _nbd_completion_callback_free (void *user_data);

extern int debug_callback (long *callbackid, char *context, char *msg);
...
extern int extent_callback (long *callbackid, char *metacontext, uint64_t offset, uint32_t *entries, size_t nr_entries, int *error);
...
extern int context_callback (long *callbackid, char *name);
