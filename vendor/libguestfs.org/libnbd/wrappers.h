...
/* Older libnbd releases do not define the 64-bit block status callback
 * types. The generated wrappers already return ENOTSUP when the related
 * functions are unavailable, so provide local stand-ins to keep cgo builds
 * working against those headers.
 */
