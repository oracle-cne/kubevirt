...
func blockStatusExtents(entries []uint32) ([]libnbd.LibnbdExtent, error) {
	if len(entries)%2 != 0 {
		return nil, fmt.Errorf("block status returned an odd number of extent entries: %d", len(entries))
	}
	...
	for currentOffset < endOffset {
		...
		err := l.BlockStatus(endOffset-currentOffset, currentOffset,
			func(metacontext string, offset uint64, entries []uint32, nbdErr *int) int {
				extents, err := blockStatusExtents(entries)
				if err != nil {
					*nbdErr = 1
					return -1
				}
